...
@TeleOp(name="follow_line_bot")
public class FollowLineBot extends OpMode {
    ...
    private double lineProjectionDistance = 10;
    private double lineProjectionPixels = 150;
...
// adjust relative speed based on heading error.
    ...
    public void findLineTargetPosition() {
        double lineRelativeAngle = ((90-pipeline.getLineAngle())/180*Math.PI+2.5*Math.PI)%Math.PI-Math.PI/2;
        telemetry.addData("Line relative angle",lineRelativeAngle);
        double lineGlobalAngle = gyroEncoderLocation.getRot()+lineRelativeAngle;
        //telemetry.addData("Line Global Angle", lineGlobalAngle);
        if (true){ //relative & angle based navigation - all numbers in pixels, not CM
            Point linePoint = pipeline.getLineCenter();
            double[] linePointRelative = {linePoint.x-pipeline.maskOutput().size().width/2,pipeline.maskOutput().size().height/2-linePoint.y};
            telemetry.addData("Relative Line Point",String.format("{0,1}",linePointRelative[0],linePointRelative[1]));
            double[] relativeTarget = {linePoint.x - Math.sin(lineRelativeAngle)*lineProjectionPixels, linePoint.y+Math.cos(lineRelativeAngle)*lineProjectionPixels};
            telemetry.addData("Relative target Point",String.format("{0,1}",relativeTarget[0],relativeTarget[1]));
            double targetDTheta = Math.atan2(relativeTarget[1],relativeTarget[0])-Math.PI/2;
            targetLocation.setRot(targetDTheta+gyroEncoderLocation.getRot());
        }
